...
console.log("initiating vault...", process.env.VAULT_URL);
...
axios
  .post(`${process.env.VAULT_URL || "http://localhost:8200"}/v1/sys/init`, {
    secret_shares: 1,
    secret_threshold: 1,
  })
  .then(async (result) => {
    ...
    // unseal vault server
    const unsealResult = await axios.post(
      `${process.env.VAULT_URL || "http://localhost:8200"}/v1/sys/unsea`,
      {
        secret_shares: 1,
        key: keys[0],
      },
  ...
  .catch((err) => {
    ...
    // trying to unseal
    const firstKey: string = JSON.parse(
      fs.readFileSync("vault-seal-keys.json").toString()
    ).keys[0];

    axios
      .post(
        `${process.env.VAULT_URL || "http://localhost:8200"}/v1/sys/unseal`,
        {
          secret_shares: 1,
          key: firstKey,
        }
