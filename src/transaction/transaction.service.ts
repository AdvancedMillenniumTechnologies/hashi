  ...
  async applicationCall(
    ...
    try {
      ...
      return {
        txnId,
        applicationId:
          transaction.transaction.createdApplicationIndex ?? appIndex,
        error: null,
      };
    } catch (error) {
        ...
        switch (txConfig.type) {
          ...
          case "application":
            ...
            const appArgs = txConfig.params.appArgs
              ? txConfig.params.appArgs.map(
                  (arg) => new Uint8Array(Buffer.from(arg))
                )
              : [];
    ...
    try {
      ...
      for (const txConfig of transactions) {
        ...
        switch (txConfig.type) {
          case "payment":
            ...
            txObject = await this.makePaymentTxn(
              from,
              paymentParams.to,
      ...
      for (let i = 0; i < encodedTxns.length; i++) {
        try {
          ...
          const ready = await this.txnCrafter.addSignature(
            encodedTxns[i],
            signedTxn
          );
          signedTxns.push(ready);
