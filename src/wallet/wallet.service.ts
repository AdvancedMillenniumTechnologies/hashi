...
export class WalletService implements OnModuleInit {
  ...
  async submitTransaction(txn: Uint8Array): Promise<string> {
    try {
      const nodeHttpScheme: string =
        this.configService.get<string>("NODE_HTTP_SCHEME");
      ...
      Logger.error(error, "WalletService.submitTransaction");
      throw error;
    }
  }
  ...
  async getPublicKey(
    keyName: string,
    keyType: "ed25519" | "ecdsa-p256" = "ed25519"
  ): Promise<Buffer> {
  ...
  async rawSign(data: Buffer, keyName: string): Promise<Buffer> {
    const signature: Buffer = await this.vaultService.sign(
      keyName,
      data,
      "sha2-512"
    );
