...
function configuredModules(): Array<
  Type<any> | DynamicModule | Promise<DynamicModule> | ForwardReference
> {
  const csv_modules_names: string[] = process.env.CHOOSEN_MODULES
    ? process.env.CHOOSEN_MODULES.split(",")
    : [];
  const modules: Array<
    Type<any> | DynamicModule | Promise<DynamicModule> | ForwardReference
  > = csv_modules_names.length == 0 ? [WalletModule] : [];
