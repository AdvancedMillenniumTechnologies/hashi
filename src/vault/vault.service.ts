...
export class VaultService {
  ...
  constructor(
    ...
    console.log(
      `VAULT_URL ----------------: ${this.configService.get("VAULT_URL")}`
    );
  }

  async auth(token: string): Promise<boolean> {
    const res: AxiosResponse = await this.httpService.axiosRef.get(
      `${this.configService.get("VAULT_URL") || "http://localhost:8200"}/v1/sys/auth`,
      {
        headers: {
          "X-Vault-Token": token,
        },
  ...
  async sign(
    ...
    const result: AxiosResponse = await this.httpService.axiosRef.post(
      `${this.configService.get("VAULT_URL") || "http://localhost:8200"}/v1/transit/sign/${keyName}`,
      {
